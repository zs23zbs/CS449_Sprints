...
for i in range(5):
    window.columnconfigure(i, weight=5, minsize= 50)
    ...
    for j in range(1,6):
        ...
        boardGame_label = tk.Label(master=boardGame_frame, width=5, height=5, text="")
...
mode_frame = tk.Frame(master=window, bd=2, relief=tk.RAISED, bg="#20B2AA")
...
for i, modes in enumerate(game_modes): # Iterate through the game_modes list to display the different versions users wants 
    tk.Radiobutton(
        ...
        value=modes,
        command = pick_mode
    ).grid(row=i+1, column=0, sticky="ew", padx=5, pady=5)
